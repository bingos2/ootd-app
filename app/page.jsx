...
const Card = ({children, banner="갈아입히기 쉬운 겨울 등원룩"}) => (
  <div style={{width:"100%",maxWidth:380,aspectRatio:"1",borderRadius:16,overflow:"hidden",background:"#4BBDE8",position:"relative",margin:"0 auto",boxShadow:"0 8px 28px rgba(75,189,232,0.4)"}}>
    ...
    <div style={{position:"absolute",bottom:0,left:0,right:0,height:58,background:"#4BBDE8",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <span style={{fontSize:"1.05rem",fontWeight:900,color:"white",fontStyle:"italic",letterSpacing:-0.5}}>{banner}</span>
    </div>
  </div>
);
...
export default function App() {
  ...
  async function generate() {
    setLoading(true); setAiData(null);
    const steps = ["🔍 옷 분석 중...","👗 아이템 파악 중...","💰 가격 추정 중...","✍️ 후기 작성 중...","📝 캡션 생성 중...","🎨 카드 조립 중..."];
    let si = 0;
    setLoadingText(steps[0]);
    loadRef.current = setInterval(() => { si++; setLoadingText(steps[si % steps.length]); }, 1100);
    ...
    try {
      const res = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({ imageBase64, mimeType, weather: w, dateStr })
      });
  ...
  function copyCaption() {
    ...
    const txt = `${aiData.instagramTitle}\n\n${aiData.instagramBody}\n\n🌡️ 오늘 날씨 ${w.temp}℃ (체감 ${w.feels}℃)\n\n${(aiData.hashtags||[]).map(h=>"#"+h).join(" ")}`;
    navigator.clipboard.writeText(txt).then(()=>alert("✅ 캡션 복사 완료!\n인스타에 바로 붙여넣기 하세요 😊"));
  }
    ...
    <div style={{fontFamily:"'Noto Sans KR',sans-serif",background:"#e8f4fd",minHeight:"100vh",paddingBottom:60}}>
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"20px 16px"}}>
        ...
        <Panel label="STEP 01" title="📸 오늘 등원룩 사진 올리기">
          {!photo ? (
            <label style={{display:"block",border:"2.5px dashed #b8dff0",borderRadius:16,padding:"32px 16px",textAlign:"center",cursor:"pointer",background:"#f0faff",transition:"all 0.2s"}}>
              <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>handleFile(e.target.files[0])}/>
              <div style={{fontSize:"2.2rem",marginBottom:8}}>👗</div>
              <div style={{fontSize:"0.85rem",color:"#888"}}><span style={{color:"#4BBDE8",fontWeight:700}}>클릭</span>해서 사진 업로드</div>
            ...
            <div style={{position:"relative"}}>
              ...
              <button onClick={()=>setPhoto(null)} style={{position:"absolute",top:10,right:10,background:"rgba(0,0,0,0.55)",color:"white",border:"none",fontSize:"0.72rem",padding:"5px 12px",borderRadius:20,cursor:"pointer"}}>✕ 다시선택</button>
            </div>
          )}
        </Panel>
        ...
        <Panel label="STEP 03" title="✨ AI 카드 + 캡션 자동 생성">
          <button disabled={!photo||loading} onClick={generate}
            style={{width:"100%",background:(!photo||loading)?"#ccc":"#4BBDE8",color:"white",border:"none",padding:16,borderRadius:14,fontSize:"1rem",fontWeight:700,cursor:(!photo||loading)?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"inherit",transition:"all 0.2s"}}>
        ...
        {aiData && <>
          ...
          <Panel label="STEP 04" title="🖼️ 인스타 카드 4장 미리보기">
            <div style={{display:"flex",gap:8,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
              {["1장 날씨","2장 아이템","3장 후기","4장 사진"].map((t,i)=>(
                <button key={i} onClick={()=>setActiveSlide(i)}
                  style={{flexShrink:0,background:activeSlide===i?"#4BBDE8":"#e8f4fd",color:activeSlide===i?"white":"#666",border:"none",padding:"6px 14px",borderRadius:20,fontSize:"0.72rem",fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
                  {t}
                </button>
              ))}
            </div>
            ...
            {activeSlide===0 && <Card>
              ...
              {(aiData.items||[]).map((it,i)=>(
                <div key={i} style={{fontSize:"0.74rem",marginBottom:5,color:"#333"}}>{it.emoji} <b>{it.type}</b> — {it.brand} · {(it.price||0).toLocaleString()}원</div>
              ))}
            ...
            {activeSlide===1 && <>
              <Card>
                {(()=>{ const it=(aiData.items||[])[activeItem]||{}; return <>
              ...
              <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:10}}>
                {(aiData.items||[]).map((_,i)=>(
                  <div key={i} onClick={()=>setActiveItem(i)} style={{width:10,height:10,borderRadius:"50%",background:activeItem===i?"#4BBDE8":"#c8e8f4",cursor:"pointer",transition:"background 0.15s"}}/>
                ))}
              </div>
            </>}
          ...
          <Panel label="STEP 05" title="✍️ 인스타 캡션 자동 생성">
            ...
            <div style={{background:"#f0faff",borderRadius:12,padding:"12px 14px",marginBottom:10}}>
              ...
              <div style={{lineHeight:2}}>{(aiData.hashtags||[]).map((h,i)=>(
                <span key={i} style={{display:"inline-block",background:"#4BBDE8",color:"white",fontSize:"0.66rem",fontWeight:700,padding:"3px 10px",borderRadius:20,margin:"2px 3px"}}>#{h}</span>
              ))}</div>
            </div>
          ...
          <button onClick={()=>{setPhoto(null);setAiData(null);setActiveSlide(0);setActiveItem(0);}}
