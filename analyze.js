exports.handler = async function(event, context) {
  ...
  try {
    ...
    if (!apiKey) {
      return {
        statusCode: 500,
        body: JSON.stringify({ ok: false, error: "ANTHROPIC_API_KEY 없음" })
      };
    }
    ...
    if (!response.ok) {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ ok: false, error: "Anthropic API 오류 (" + response.status + "): " + errMsg })
      };
    }
    ...
    try {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ ok: false, error: "JSON 파싱 실패: " + txt.slice(0, 200) })
      };
    }
